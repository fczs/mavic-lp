...
if(isset($_POST['images-edit-newval'])){
     set_option($_POST['name'], $_POST['images-edit-newval']);
}

if(isset($_GET['images-edit'])){
	...
	$file = $val["name"];
	echo "<images class=\"selector\" src=\"$file\" />";
    }

}

function sortfunc($a, $b){ return $a["timestamp"] < $b["timestamp"]; }

addjs(JQUERY);
addjs(JQUERYFORM);
addjs(JQUERYUI_JS);
addjs("components/images-edit.js");
addjs("components/jcrop.min.js");

addcss(JQUERYUI_CSS);
addcss("components/jcrop.css");
addcss("components/images-edit.css");
...
function img_edit($key, $folder, $w, $h, $description, $save_orig = ""){
    ...
    echo '<div '.$scrolltome.'class="images-edit" name="'.$key.'" folder="'.$folder.'" save-orig="'.$save_orig.'" w="'.$w.'" h="'.$h.'" desc="'.$description.'" ><div class="pencil"></div><div class="desc">'.$description.'</div></div>';
}
