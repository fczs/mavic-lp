...
if(isset($_FILES['imgupload'])){
    ...
    if( ($size[0] > 1300) or ($size[1]>5000) )
	system("convert -resize 1300x5000 $src $tfpath");
    else
    ...
    echo '<images class="srcimg" '.$size[3].' src="?ajax&tempimg&rand=' . time() .'" />';
}
...
function imgupload_footer(){
    echo '<div id="images-upload-footer-dialog">' . "\r\n";
    imgupload_form();
    echo "</div>\r\n";
}
