...
function gi($key, $path, $width, $height)
{
    img_edit($key, $path, $width, $height, "Изменить картинку");
    echo '<images src="' . g($key) . '" />';
}

function gf($key, $path, $width, $height)
{
    img_edit($key, $path, $width, $height, "Изменить картинку", "orig.jpg");
    echo '<a href="' . g($key) . 'orig.jpg" class="fancybox" data-fancybox-group="gr' . $path . '"><images src="' . g($key) . '" /></a>';
}
...
//TODO: change get host
function get_host()
{
    $host = strtolower($_SERVER['HTTP_HOST']);
    if (preg_match("/^www\./", $host))
        $host = substr($host, 4);
    return "mavic.spb.ru";//$host;
}
